...
log_file_name = logs_dir + 'hops_' + str(FLAGS.hops) + '_l2_' + str(
    FLAGS.regularization) + '_embed_size_' + str(FLAGS.embedding_size) + '_' + FLAGS.output_file
...
with tf.Session() as sess:
    ...
    for i in range(1, FLAGS.epochs + 1):
        ...
        if i == FLAGS.epochs:
            print('Writing final results to {}'.format(log_file_name))
            train_accs = [float("{0:.2f}".format(i)) for i in train_accs]
            val_accs = [float("{0:.2f}".format(i)) for i in val_accs]
            test_accs = [float("{0:.2f}".format(i)) for i in test_accs]
            df = pd.DataFrame({
                'Train': train_accs,
                'Val': val_accs,
                'Test': test_accs
            }, index=range(1, 21), columns=['Train', 'Val', 'Test'])
            df.index.name = 'Task'
