...
class MemN2N(object):
    ...
    def _build_vars(self):
        with tf.variable_scope(self._name):
            ...
            tf.add_to_collection('reg_loss', tf.nn.l2_loss(self.A_1))

            self.C = []

            for hopn in range(self._hops):
                with tf.variable_scope('hop_{}'.format(hopn)):
                    C_layer = tf.Variable(C, name="C")
                    tf.add_to_collection('reg_loss', tf.nn.l2_loss(C_layer))
                    self.C.append(C_layer)
    ...
    def predict_prob_instrument(self, stories, queries):
        """Predicts probabilities of answers.
        ...
        """
        feed_dict = {self._stories: stories, self._queries: queries}
        return self._sess.run([self.predict_op, self.predict_proba_op, self.prob_hops, self.A_1, self.C],
                              feed_dict=feed_dict)
